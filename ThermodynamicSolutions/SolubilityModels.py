...
class dataset:
    ...
    def experimental_values(self,scale = 0,download_format = "None"):
        ...
        In the LaTex format the output can be copy/pasted into a main LaTeX document, 
        requires `\\usepackage{booktabs}`. Default is ‘None’.
        """
        ...
        nombre ="experimental values_Modified Apelblat"
...
#CLASE PARA EL MODELO DE SOLUBILIDAD APELBLAT MODIFICADO
    ...
    class modified_apelblat(dataset):
        ...
        def values(self,method="lm",p0 =[1,1,1], maxfev=20000,scale = 0,download_format = "None"):
            ...
            LaTeX format (‘tex’). In the LaTex format the output can be copy/pasted into a main 
            LaTeX document, requires `\\usepackage{booktabs}`. Default is ‘None’.
            """
        ...
        def plot(self,method="lm",p0 =[1,1,1], maxfev=20000,apart = False,download_format = "pdf"):
            ...
            LaTeX format (‘tex’). In the LaTex format, if `apart = True` requires the preambule:
            `\\usepgfplotslibrary{groupplots}`
            `\\pgfkeys{/pgf/number format/.cd,fixed,precision=3}`
            `\\pgfplotsset{compat=1.3,every axis/.append style={scale only axis,height=5.5cm,width=6cm}}`
            For the distance between the graphs use:
            `\\begin{groupplot}[group style={horizontal sep=2cm, vertical sep=3cm}]`
            """
...
#CLASE PARA EL MODELO DE SOLUBILIDAD VAN’T HOFF
    class vant_hoff(dataset):
        ...
        def parameters(self,funtion = "fx", method="lm",p0 =[1,1], maxfev=20000, sd = False, cg = True, cmap="Blues",download_format = "None"):
            ...
            Option to download the dataframe of experimental values in the chosen format,
            excel format (‘xlsx’), comma separated values format (‘csv’), LaTeX format (‘tex’).
            In the Latex format requires `\\usepackage{booktabs}`. The output can be copy/pasted
            into a main LaTeX document. Default is ‘None’.
            """
            idx = pd.IndexSlice
            slice_1 = idx[idx[:], idx["MAPE"]]
...
class models(model.modified_apelblat,model.vant_hoff,model.vant_hoff_yaws,model.modified_wilson,model.buchowski_ksiazaczak):
    ...
    def statisticians(self,Tf = " ",ΔHf= " ",method="lm",p0_ma =[1,1,1],p0_vh =[1,1],p0_vhy =[1,1,1],p0_mw =[1,1],p0_bk=[1,1],maxfev=20000):
        ...
        if Tf != " ": 
            ...
            names_model.append("Buchowski_λh")
            std.append(std_5)
        ...
        dataframe = pd.DataFrame(dict(zip(names_model,std))).style.highlight_max(subset=slice_1,axis=1, props='color:white; font-weight:bold; background-color:#4191c6;')\
                                                                  .highlight_min(subset=slice_2,axis=1, props='color:white; font-weight:bold; background-color:#4191c6;')\
                                                                  .highlight_min(subset=slice_1,axis=1, props='color:black; font-weight:bold; background-color:#cadef0;')\
                                                                  .highlight_max(subset=slice_2,axis=1, props='color:black; font-weight:bold; background-color:#cadef0;')\
